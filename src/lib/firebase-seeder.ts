...
import { UserProfile, Astronaute, Onboarding, Session, Score, Grade, Promotion, Report } from '../types';
...
export const DEFAULT_PROFILES: UserProfile[] = [
  ...
  // Adding user email dynamically to enable immediate full admin dev logging or any default email
  { id: 'justin_admin', email: 'justinberthonn@gmail.com', full_name: 'Justin Berthonn', role: 'developer', can_enter_data: true, pin: '9999', assignment: null }
];

export const DEFAULT_ASTRONAUTES: Astronaute[] = [
...
export const DEFAULT_PROMOTIONS: Promotion[] = [
  ...
  // Toussaint (fully validated up to rank 18)
  ...DEFAULT_GRADES.map((g, index) => ({
    id: `prom_toussaint_${index}`,
...
/**
 * Automagic Firestore database and collections seeder. Matches the exact Express db.json fallback structures.
 */
export async function seedFirestoreIfEmpty() {
  let querySnapshot;
  try {
    const profilesQuery = query(collection(db, 'profiles'), limit(1));
    querySnapshot = await getDocs(profilesQuery);
  } catch (err: any) {
    console.error("Critical error testing/reading profiles collections on seeding check:", err);
  ...
  console.log("Firestore empty! Commencing automated high-faith full seeding...");
  ...
  try {
    ...
    // We can use Batches to safely seed up to 500 records at a time
    // 2. Profiles
    const batchProfiles = writeBatch(db);
    DEFAULT_PROFILES.forEach(p => {
      batchProfiles.set(doc(db, 'profiles', p.id), p);
    });
    await batchProfiles.commit();

    // 3. Astronautes
    const batchAstros = writeBatch(db);
    DEFAULT_ASTRONAUTES.forEach(a => {
      batchAstros.set(doc(db, 'astronautes', a.id), a);
    });
    await batchAstros.commit();

    // 4. Onboarding
    const batchOnboarding = writeBatch(db);
    DEFAULT_ONBOARDING.forEach(o => {
      batchOnboarding.set(doc(db, 'onboarding', o.astronaute_id), o);
    });
    await batchOnboarding.commit();

    // 5. Grades (Static ranks definition)
    const batchGrades = writeBatch(db);
    DEFAULT_GRADES.forEach(g => {
      batchGrades.set(doc(db, 'grades', g.id), g);
    });
    await batchGrades.commit();

    // 6. Promotions
    const batchProms = writeBatch(db);
    DEFAULT_PROMOTIONS.forEach(p => {
      batchProms.set(doc(db, 'promotions', p.id), p);
    });
    await batchProms.commit();

    // 7. Sessions
    const batchSessions = writeBatch(db);
    DEFAULT_SESSIONS.forEach(s => {
      batchSessions.set(doc(db, 'sessions', s.id), s);
    });
    await batchSessions.commit();

    // 8. Scores
    // Scores might exceed 100 elements, let's keep it safe with batch chunks
    const batchScores = writeBatch(db);
    DEFAULT_SCORES.forEach(sc => {
      batchScores.set(doc(db, 'scores', sc.id), sc);
    });
    ...
    DEFAULT_REPORTS.forEach(r => {
      batchReports.set(doc(db, 'reports', r.id), r);
    });
    await batchReports.commit();

    console.log("Firestore fully seeded with default mock structure successfully!");
  } catch (error: any) {
    console.error("Critical error during Firestore database seeding:", error);
